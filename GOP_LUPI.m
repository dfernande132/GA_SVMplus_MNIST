...
% function GOP_LUPI 
function [out_GOP, out_POP] = GOP_LUPI(fv,fvStar,lbl,depth)
clc;
if nargin == 3
    depth=1;
elseif nargin ~= 4
...
switch depth
    case 1
        CparamPlus_ini  =  [0.01, 100];
        gammaParam_ini  =  [0.01, 100];
        sgmPlus_ini    =   [.1, 10];  % Small sigmas-->overfitting
        sgmStar_ini    =   [.1, 10];  % Large Sigmas-->smoothing
        numpartesC=4;
        numpartesG=4;
        numpartesSP=2;
        numpartesSS=2;
...
generations =200*depth; % number of generations 
...
function [fitness] = evalua_fitness(pop,fv,fvStar,lbl)
    ...
    for i=1 : n
        ...
        [valPlus, ~, ~, ~, result] = FOM_LUPI(fv, fvStar, lbl, pop(i,1),pop(i,2),pop(i,3),pop(i,4));
